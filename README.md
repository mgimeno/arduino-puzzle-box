...
5 red LEDs are lit up as soon as we switch on the box (by plugging in a micro USB), signalling the number of puzzles remaining to be solved.
...
To lock the box after it's been opened, we just need to close the lid and then push any of the buttons under the RGB LED or any of the buttons on the keypad. The servomotor will move to lock the box.
...
Built using the official Arduino IDE.
